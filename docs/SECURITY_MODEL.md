...
Browser sessions default to `local_fetch_controller` mode. Operators can explicitly enable `playwright_controller` with `PLATFORM_ENABLE_PLAYWRIGHT_RUNNER=1` after installing Playwright locally. The Playwright path executes JavaScript and captures rendered screenshots, visible text, console messages, and network summaries, but it does not bypass policy: the requested target is checked before navigation, renderer subrequests are blocked when they fall outside `ScopePolicy`, and the final URL is checked before any evidence is stored. Screenshots remain `raw_local_only`; DOM text, console messages, network URLs, headers, and session URLs are redacted before persistence. TLS MITM, local CA trust, video/trace retention, and authenticated secret injection remain separate desktop Runner capabilities, not implicit browser-session powers.
