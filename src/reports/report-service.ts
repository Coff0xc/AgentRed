...
import { AdvancedReportFormatter } from './advanced-report-formatter.js';

export type ReportFindingScope = 'confirmed_only' | 'candidate_and_confirmed';
export type ReportFormat = 'hackerone' | 'bugcrowd' | 'src' | 'enterprise' | 'executive';
...
export class ReportService {
  ...
  generate(input: {
    ...
    const sortedFindings = AdvancedReportFormatter.sortFindingsBySeverity(findings);

    let markdown: string;
    if (input.format === 'executive') {
      // Executive format with advanced features
      markdown = this.generateExecutiveReport(snapshot, sortedFindings, findingScope);
    } else {
      // Legacy format
      markdown = this.generateLegacyReport(snapshot, sortedFindings, findingScope, input.format);
    }
  ...
  private generateExecutiveReport(snapshot: any, findings: Finding[], findingScope: ReportFindingScope): string {
    const sections: string[] = [
      ...
      '---',
      '',
      AdvancedReportFormatter.generateExecutiveSummary(snapshot, findings),
      AdvancedReportFormatter.generateSeverityChart(findings),
      AdvancedReportFormatter.generateRiskSummary(findings),
    ];

    if (findings.length > 0) {
      sections.push('## Detailed Findings', '');
      findings.forEach((finding, index) => {
        sections.push(AdvancedReportFormatter.formatEnhancedFinding(finding, index + 1));
      });
    } else {
      sections.push('## Detailed Findings', '', 'No findings match the selected report scope.', '');
    }

    const attackMatrix = AdvancedReportFormatter.generateAttackMatrix(findings);
    if (attackMatrix) {
      sections.push(attackMatrix);
    }
    ...
    sections.push(
      ...
      `**Allowed Methods**: ${snapshot.run.scopePolicy.allowedMethods.join(', ')}`,
      '',
      '### Evidence Policy',
      '',
      'Every finding in this report references reproducible evidence. Raw local-only evidence is not uploaded to cloud control plane by default.',
      '',
      '---',
      '',
      `*Report generated by AgentRed v${this.getVersion()}*`,
    );

    return sections.join('\n');
  }

  private generateLegacyReport(snapshot: any, findings: Finding[], findingScope: ReportFindingScope, format: string): string {
    return [
      '# AgentRed Report',
      '',
      `**Format**: ${format}`,
