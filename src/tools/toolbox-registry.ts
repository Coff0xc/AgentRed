...
export const SCANNER_TEMPLATES: ToolTemplateProfile[] = [
  ...
  {
    id: 'web.auth_endpoint_discovery',
    name: 'Auth Endpoint Discovery',
    description: 'Probe common authentication and identity endpoints (/login, /signin, /api/auth, /oauth/authorize) and record status and headers.',
    domain: 'web',
    engine: 'builtin',
    profileId: 'builtin.web',
    executionMode: 'builtin',
    ...
    defaultRiskLevel: 'R1',
    evidenceKind: 'command_output',
    riskNotes: ['Bounded HEAD requests to well-known auth paths', 'No credentials sent', 'No form submission'],
  },
  {
    id: 'web.api_version_discovery',
    name: 'API Version Discovery',
    description: 'Probe common API versioning paths (/v1, /v2, /api/v1, /rest/v2, /api/v1.0) to surface older or parallel API surfaces.',
    domain: 'web',
    engine: 'builtin',
    profileId: 'builtin.web',
    executionMode: 'builtin',
    ...
    defaultRiskLevel: 'R1',
    evidenceKind: 'command_output',
    riskNotes: ['Bounded HEAD requests to version path candidates', 'No API operation execution'],
  },
  {
    id: 'web.host_header_probe',
    name: 'Host Header Injection Probe',
    description: 'Send requests with synthetic Host headers and record response differential signals for host header injection reconnaissance.',
    domain: 'web',
    engine: 'builtin',
    profileId: 'builtin.web',
    executionMode: 'builtin',
    ...
    defaultRiskLevel: 'R2',
    evidenceKind: 'command_output',
    riskNotes: ['Sends one synthetic Host header per probe', 'No real credential or session involved', 'Records response differential only'],
  },
  {
    id: 'web.param_probe',
    name: 'Query Parameter Probe',
    description: 'Mutate existing query parameters with bounded markers and quote/timing probes to capture reflection, error, and response differential signals.',
    domain: 'web',
    engine: 'builtin',
    profileId: 'builtin.web',
    executionMode: 'builtin',
    ...
    evidenceKind: 'command_output',
    riskNotes: [
      'Only mutates parameters already present in the target URL',
      'Uses bounded GET requests and records differential signals only',
      'Does not submit forms, authenticate, or use destructive payloads',
    ],
  },
  {
  ...
  },
  {
    ...
    description: 'Nuclei adapter for curated low-risk templates behind scope, rate, and approval gates. Requires container.web-recon profile (PLATFORM_ENABLE_CONTAINER_TOOLBOX=1).',
    domain: 'web',
    engine: 'nuclei',
  ...
  },
  {
    ...
    description: 'httpx adapter for bounded HTTP service fingerprinting behind scope and rate gates. Requires container.web-recon profile (PLATFORM_ENABLE_CONTAINER_TOOLBOX=1).',
    domain: 'web',
    engine: 'httpx',
    profileId: 'container.web-recon',
    executionMode: 'external',
  ...
  },
  {
    ...
    description: 'ffuf adapter for bounded content discovery with rate limits and scope checks. Requires container.web-recon profile (PLATFORM_ENABLE_CONTAINER_TOOLBOX=1).',
    domain: 'web',
    engine: 'ffuf',
    profileId: 'container.web-recon',
    executionMode: 'external',
  ...
  },
  {
    ...
    description: 'sqlmap verification adapter for approved, narrow, evidence-backed injection checks. Requires container.web-recon profile (PLATFORM_ENABLE_CONTAINER_TOOLBOX=1) and human approval (R3).',
    domain: 'web',
    engine: 'sqlmap',
    profileId: 'container.web-recon',
    executionMode: 'external',
  ...
  },
  {
    ...
    description: 'nmap adapter for bounded top-port validation with strict scope and rate controls. Requires container.network-recon profile (PLATFORM_ENABLE_CONTAINER_TOOLBOX=1).',
    domain: 'network',
    engine: 'nmap',
  ...
  },
  {
    ...
    description: 'tlsx adapter for certificate metadata collection across approved assets. Requires container.network-recon profile (PLATFORM_ENABLE_CONTAINER_TOOLBOX=1).',
    domain: 'network',
    engine: 'tlsx',
    profileId: 'container.network-recon',
    executionMode: 'external',
  ...
  },
  {
    ...
    description: 'Semgrep adapter for local source-code SAST. Set PLATFORM_ENABLE_LOCAL_SAST=1 and PLATFORM_SAST_WORKSPACE to an explicit source root; optional PLATFORM_SEMGREP_CONFIG selects the rule config.',
    domain: 'sast',
    engine: 'semgrep',
    ...
    defaultRiskLevel: 'R1',
    evidenceKind: 'command_output',
    riskNotes: ['Requires explicit local workspace input', 'Findings must reference source evidence', 'Runs with Semgrep metrics disabled'],
  },
  {
...
export const TOOLBOX_BUNDLES: ToolboxBundle[] = [
  {
    ...
    templateIds: [
      ...
      'web.auth_endpoint_discovery',
      'web.api_version_discovery',
      'web.host_header_probe',
      'web.param_probe',
    ],
    riskLevels: ['R1', 'R2'],
    safetyNotes: [
